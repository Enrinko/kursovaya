<!DOCTYPE html> <?php include_once "../php/db.php";
...
<body onload="moreActions('<?php if (isset($_GET['theme'])) echo implode(' ', $arrayOfQuestions) ?>')"> <?php include 'header.html' ?>
<div class="title text">Добавить тест</div>
<div class="test text">
    <div class='form-div'><select class='div-select text'
                                  id='subject'> <?php $items = mysqli_query($link, "SELECT subject_name FROM subjects");
    ...
    <div class="question-with-answer">
        <div class="deleteQuestion">
            <button class="deleteQuestion" onclick="deleteSomething(this)" value="1"><img class="img trashcan-img"
                                                                                          src="../img/trashcan.png">
            </button>
        </div>
        <div class="types">
            <div><input class="typeOfAnswer multi" name="only"
                        onchange="checkTypes(this.value, this.closest('.question-with-answer'))" type="radio" value="2">
                <span>2+</span></div>
            <div><input checked class="typeOfAnswer only" name="only"
                        onchange="checkTypes(this.value, this.closest('.question-with-answer'))" type="radio" value="1">
                <span>1</span></div>
        </div>
        <input class="question-text input" name="question-text" placeholder="Тема вопроса" type="text">
        <button class="addAnswer button" onclick="add('question-answer', 'question-with-answer', this)" value="0"><img
                    class="img addA-span" src="../img/delete.png"> <span>Добавить ответ</span></button>
        <div class="question-answer"><input class="right" name="right" onchange="setRight(this)" type="radio" value="0">
            <div class="answer-input"><input class="input answer-text" name="answer" placeholder="Вопрос" type="text">
            </div>
            <div class="deleteAnswer">
                <button onclick="deleteSomething(this)" value="0"><img class="img delete-img" src="../img/delete.png">
                </button>
            </div>
        </div>
    </div>
...
<div class="actions text">
    ...
    <div class="block-of-buttons">
        <button class="addQuestion button" onclick="add('question-with-answer', 'test', this)" value="1"><img
                    class="img add-img" src="../img/delete.png"> <span class="actions-add"> Добавить вопрос </span>
        </button>
    </div>
    <form action="" method="post"><input class="save text" onclick="toPHP()" type="submit" value="Сохранить"></form>
</div>
</body>
</html> <?php
$nameOfCookie = 's';
